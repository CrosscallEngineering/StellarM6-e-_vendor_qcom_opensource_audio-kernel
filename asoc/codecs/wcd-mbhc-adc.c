...
static int wcd_measure_adc_once(struct wcd_mbhc *mbhc, int mux_ctl)
{
	...
	if (retry <= 0) {
		...
		pr_info("%s: adc complete: %d, adc timeout: %d output_mV: %d, mux_ctl=%d\n",
...
static bool wcd_is_special_headset(struct wcd_mbhc *mbhc)
{
	...
	while (!is_spl_hs) {
		...
		if (delay == SPECIAL_HS_DETECT_TIME_MS) {
			pr_info("%s: Spl headset not found in 2 sec\n",
				 __func__);
			break;
		}
	}
...

{
	...
	if (adc_result < hph_thr)
		plug_type = MBHC_PLUG_TYPE_HEADPHONE;//2
	else if (adc_result > hs_thr)
		plug_type = MBHC_PLUG_TYPE_HIGH_HPH;//3
	else
		plug_type = MBHC_PLUG_TYPE_HEADSET;//1

	pr_info("%s: plug type is %d found, hs_thr=%u, hph_thr=%d\n", __func__, plug_type, hs_thr, hph_thr);

	return plug_type;
}

#if IS_ENABLED(CONFIG_VND_AUDIO_MBHC_DET_WR)
/* vendor add: check if plug type changed during correct_plug_type process,
** if changed, report jitter, to avoid detection error induced by button press.
**/
static int is_exist_plug_type_jitter(int pt_gnd_mic_swap_cnt, int no_gnd_mic_swap_cnt, enum wcd_mbhc_plug_type plug_type, struct wcd_mbhc *mbhc) 
{
	int i = 0, jitter = 0;
	static enum wcd_mbhc_plug_type pt_gnd_plug_type[GND_MIC_USBC_SWAP_THRESHOLD] = {MBHC_PLUG_TYPE_NONE};
	static enum wcd_mbhc_plug_type no_gnd_plug_type[GND_MIC_USBC_SWAP_THRESHOLD] = {MBHC_PLUG_TYPE_NONE};

	if(pt_gnd_mic_swap_cnt<=mbhc->swap_thr && pt_gnd_mic_swap_cnt>0 && !no_gnd_mic_swap_cnt) {
		pt_gnd_plug_type[pt_gnd_mic_swap_cnt-1] = plug_type;

		for(i=1; i<pt_gnd_mic_swap_cnt; i++) {
			jitter |= (pt_gnd_plug_type[i-1]!=pt_gnd_plug_type[i]) ? 1 : 0;
			pr_err("%s: pt_gnd_mic_swap_cnt=%d, pt_gnd_plug_type=%d, prev_plug_type=%d, jitter=%d\n", \
					__func__, pt_gnd_mic_swap_cnt, pt_gnd_plug_type[i], pt_gnd_plug_type[i-1], jitter);
		}
	} else if (no_gnd_mic_swap_cnt<=mbhc->swap_thr && no_gnd_mic_swap_cnt>0 && !pt_gnd_mic_swap_cnt) {
		no_gnd_plug_type[no_gnd_mic_swap_cnt-1] = plug_type;

		for(i=1; i<no_gnd_mic_swap_cnt; i++) {
			jitter |= (no_gnd_plug_type[i-1]!=no_gnd_plug_type[i]) ? 1 : 0;
			pr_err("%s: no_gnd_mic_swap_cnt=%d, no_gnd_plug_type=%d, prev_plug_type=%d, jitter=%d\n", \
					__func__, no_gnd_mic_swap_cnt, no_gnd_plug_type[i], no_gnd_plug_type[i-1], jitter);
		}
	}
	return jitter;
}
#endif /* CONFIG_VND_AUDIO_MBHC_DET_WR */
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	int hs_threshold, micbias_mv;
#if IS_ENABLED(CONFIG_VND_AUDIO_MBHC_DET_WR)
	int plug_type_jitter = 0, recheck = 0, abnormal_check = 0;
#endif /* CONFIG_VND_AUDIO_MBHC_DET_WR */
	...
	/*vendor modified, if current type is HEADSET, immediately return*/
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		goto enable_supply;
	}
...
#if IS_ENABLED(CONFIG_VND_AUDIO_MBHC_DET_WR)
		plug_type_jitter = is_exist_plug_type_jitter(pt_gnd_mic_swap_cnt, no_gnd_mic_swap_cnt, plug_type, mbhc);

		//check twice more, if plug type changed.
		if ((pt_gnd_mic_swap_cnt || no_gnd_mic_swap_cnt) && plug_type_jitter==1 && recheck < 2) {
			timeout = jiffies + msecs_to_jiffies(HS_DETECT_PLUG_TIME_MS);
			recheck++;
			pr_err("%s: plug type changed, ignore current plug type %d, check again\n", __func__, plug_type);
			continue;
		} else {
			recheck = 0;
		}
...
#if IS_ENABLED(CONFIG_VND_AUDIO_MBHC_DET_WR)
	//if got unexpected plug type, redo correct_plug_type process to avoid detecting wrong type. maybe useful when insertion not good.
	if(!abnormal_check && ((MBHC_PLUG_TYPE_GND_MIC_SWAP==plug_type) || (MBHC_PLUG_TYPE_HIGH_HPH==plug_type && output_mv >= 2780))) {
		pr_err("%s: unexpected plug type, recheck plug type\n", __func__);
		abnormal_check++;
		goto correct_plug_type;
	}
	else 
