				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		/***  @} */
#if IS_ENABLED(CONFIG_QCOM_FSA4480_HEADSET)	
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_L_DET_EN, 0);  //vendor added, disable LDET	
#endif			 
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
...
static void wcd_btn_lpress_fn(struct work_struct *work)
{
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET) {
		pr_info("%s: Reporting long button press event, btn_result: %d\n",
			 __func__, btn_result);/*vendor modified*/

#if IS_ENABLED(CONFIG_VND_AUDIO_HEADSET_PTT_KEY)
		//set SLV_BOLERO_EVT_BCS_CLK_OFF to 0 for ptt btn to solve ptt mic mute issue
		if(btn_result == 4) {
			if (mbhc->mbhc_cb->bcs_enable)
				mbhc->mbhc_cb->bcs_enable(mbhc, false); 
		}
#endif /* CONFIG_VND_AUDIO_HEADSET_PTT_KEY */
...
static irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	/*vendor modified,if headset is not reported,do not handle the button press irq*/
	...
	if (schedule_delayed_work(&mbhc->mbhc_btn_dwork,
				msecs_to_jiffies(100)) == 0) { //L79015-2562: reduce 400ms-> 100ms
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			pr_info("%s: Reporting long button release event, buttons_pressed=0x%x\n",
				 __func__, mbhc->buttons_pressed);/*vendor modified*/

#if IS_ENABLED(CONFIG_VND_AUDIO_HEADSET_PTT_KEY)
			//set SLV_BOLERO_EVT_BCS_CLK_OFF to 1 when release ptt btn, to avoid high-freq noise
			if (mbhc->buttons_pressed & SND_JACK_BTN_4) { //0x400
				if (mbhc->mbhc_cb->bcs_enable)
		...
		unsigned long mode, void *ptr)
{
	...
	if (usbc_det_status != usbc_temp){
		...
		/*vendor modified*/
		pr_info("%s: usbc_det_status = %d, headset_status=%d\n", __func__, usbc_det_status, headset_status);
		if (mbhc == NULL) {
			pr_err("%s: NULL irq data\n", __func__);
			return -EINVAL;
		}
