					    ...
					    bool bcs_disable)
{
	...
	if (wcd937x->update_wcd_event) {
		...
		pr_info("%s: bcs_disable=%d\n", __func__, bcs_disable);
				...
				void *data)
{
	...
	case BOLERO_SLV_EVT_SSR_DOWN:
		wcd937x->mbhc->wcd_mbhc.deinit_in_progress = true;
		 /*vendor add to save the ADC volume before Modem reboot.*/
		adc1_vol=snd_soc_component_read(component, WCD937X_ANA_TX_CH1);
		adc2_vol=snd_soc_component_read(component, WCD937X_ANA_TX_CH2);
		adc3_vol=snd_soc_component_read(component, WCD937X_ANA_TX_CH3);
	...
	case BOLERO_SLV_EVT_SSR_UP:
		...
		/*vendor add to rewrite the ADC volume after Modem reboot.*/
					  ...
					  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		wcd937x_micbias_control(component, micb_num,
				MICB_ENABLE, true);
		/*** BSP : added by vendor to set  micbias1/3 to 2.8v @{ */
#if IS_ENABLED(CONFIG_VND_AUDIO_MICBIAS_2POINT8)
		...
		if (micb_num == MIC_BIAS_2) {
			snd_soc_component_update_bits(component, WCD937X_ANA_BIAS, 0x40, 0x40);   
			snd_soc_component_update_bits(component, WCD937X_ANA_MICB2, 0x3F, 0x24);   
		}
		if (micb_num == MIC_BIAS_3) { 
			snd_soc_component_update_bits(component, WCD937X_ANA_BIAS, 0x40, 0x40);   
			snd_soc_component_update_bits(component, WCD937X_ANA_MICB3, 0x3F, 0x24);  
		}
#endif /*CONFIG_VND_AUDIO_MICBIAS_2POINT8*/
		break;
	case SND_SOC_DAPM_POST_PMU:
