...
#define WCD_MBHC_HS_V_MAX           1700 //vendor added, for factory test headset check fail
...
#if IS_ENABLED(CONFIG_VND_AUDIO_HEADSET_PTT_KEY)
	.key_code[4] = KEY_HEADSET_PTT,
#else /*CONFIG_VND_AUDIO_HEADSET_PTT_KEY*/
	.key_code[4] = 0,
#endif /*CONFIG_VND_AUDIO_HEADSET_PTT_KEY*/
	.key_code[5] = 0,
	.key_code[6] = 0,
...
static bool msm_usbc_swap_gnd_mic(struct snd_soc_component *component, bool active)
{
	...
	return (fsa4480_switch_event(pdata->fsa_handle, FSA_MIC_GND_SWAP)==0) ? true : false;
}
...
static void *def_wcd_mbhc_cal(void)
{
	...
	btn_high[1] = 76;
	btn_high[2] = 200;
	/*vendor added, for headset PTT key*/
#if IS_ENABLED(CONFIG_VND_AUDIO_HEADSET_PTT_KEY)
	btn_high[3] = 450;//calculated value=425, set 450 to align with PTT-disabled feature
	btn_high[4] = 790;
	btn_high[5] = 790;
	btn_high[6] = 790;
	btn_high[7] = 790;
...
#if IS_ENABLED(CONFIG_VND_REMOVE_WSA)
...
#if IS_ENABLED(CONFIG_SND_SOC_AW87XXX)
	ret = aw87xxx_add_codec_controls(lpass_cdc_component);
	if (ret < 0) {
		pr_err("%s: add_aw87XXX_codec_controls failed, err %d\n",
				__func__, ret);
		return ret;
	};
#endif /*CONFIG_SND_SOC_AW87XXX*/
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		dev_info(&pdev->dev,"%s: wsa-max-devs support = %d\n", __func__, pdata->wsa_max_devs);
